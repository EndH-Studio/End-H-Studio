...
document.querySelectorAll('.nav-link, .logo').forEach(link => {
  link.addEventListener('click', function(e) {
    ...
    // Ambil target tab (misal: "home")
    const targetTab = this.getAttribute('data-tab');
    
    if (targetTab) {
      // 1. Sembunyikan semua halaman tab
      document.querySelectorAll('.tab-page').forEach(page => {
        page.classList.remove('active');
      });
      
      // 2. Tampilkan halaman target (Home)
      const activePage = document.getElementById(targetTab);
      if (activePage) {
        activePage.classList.add('active');
      }

      // 3. Reset status menu navigasi aktif ke "Beranda"
      document.querySelectorAll('nav.main-nav a').forEach(nav => {
      ...
      // 4. Scroll halus ke paling atas halaman
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  });
});
